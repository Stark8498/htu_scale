module TRINH_VAN_PHUC::HTU_ScalePlus::RadialMenu
  class LabelText
    ...
    def initialize(text, leader_type = nil, target_point = nil)
      @text = text
      @position = [0, 0]
      @options = {:font => "Arial", :size => 12, :bold => false, :rounded => true, :align => TextAlignLeft, :vertical_align => TextVerticalAlignCenter, :color => "blue", :background_color => "white", :border_width => 1, :border_color => "blue", :padding => 12}
      ...
      @options[:padding] = @options[:size] / 2
    ...
    def redraw
      ...
      x1 = x1 - @options[:padding]
      y1 = y1 - @options[:padding] / 4
      x2, y2 = @bounds.lower_right.to_a
      x2 = x2 + @options[:padding]
      y2 = y2 + @options[:padding]
      @rect = [Geom::Point3d.new(x1, y1 - @options[:padding] / 4), Geom::Point3d.new(x1, y2 - @options[:padding] / 4), Geom::Point3d.new(x2, y2 - @options[:padding] / 4), Geom::Point3d.new(x2, y1 - @options[:padding] / 4)]
      @shape = @options[:rounded] ? rounded_shape(@rect, @options[:size] / 2) : @rect
